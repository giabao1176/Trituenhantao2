...
def solve_astar(M, N, start_pos, initial_dirty, obstacles, style):
    ...
    while frontier:
        ...
        step_log = f"Bước {nodes_expanded}: Xét node ({r},{c}) [f={f}, g={g}, h={h}], còn {len(dirty)} ô bụi"
        ...
        for dr, dc in directions:
            ...
            if 0 <= nr < M and 0 <= nc < N and (nr, nc) not in obstacles:
                ...
                if next_state not in best_cost or next_g < best_cost[next_state]:
                    ...
                    heapq.heappush(frontier, (next_f, tie_breaker, next_g, next_state))
...
def solve_astar_stepwise(M, N, start_pos, initial_dirty, obstacles, style):
    ...
    while frontier:
        ...
        frontier_show = ", ".join(f"({s[3][0][0]},{s[3][0][1]})[f={s[0]},g={s[2]}]" for s in sorted_frontier[:5])
        if len(frontier) > 5:
            frontier_show += f" ... (+{len(frontier)-5} node)"

        log_text = f"Bước {nodes_expanded}: Xét node ({r},{c}) [f={f}, g={g}, h={h}], còn {len(dirty)} ô bụi\n  -> Frontier: [{frontier_show}]"
