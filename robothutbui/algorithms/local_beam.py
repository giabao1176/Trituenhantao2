...
def solve_local_beam(M, N, start_pos, initial_dirty, obstacles, style):
    ...
    while step < max_steps:
        ...
        # Check if any beam is already at the goal state
        for beam in beams:
            if not beam["dirty"]:
        ...
        # Expand all beams to get candidates for the next step
        candidates = []
        for i, beam in enumerate(beams):
            ...
            for dr, dc in directions:
                ...
                if 0 <= nr < M and 0 <= nc < N and (nr, nc) not in obstacles:
                    ...
                    # Prevent looping back in the same beam
                    if state_key not in beam["visited"]:
                        ...
                        # Style 2 (check goal when generating node)
                        if not next_dirty:
        ...
        # Select k best successors (filter duplicates first)
        seen = set()
        unique_candidates = []
        ...
        # Sort unique candidates by heuristic h
        unique_candidates.sort(key=lambda x: x["h"])
        
        # Take the top k
        beams = unique_candidates[:k]
        
        # Log the current status of the beams
        best_beam = beams[0]
