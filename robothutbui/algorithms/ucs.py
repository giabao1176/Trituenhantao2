...
def solve_ucs(M, N, start_pos, initial_dirty, obstacles, style):
    ...
    while frontier:
        ...
        step_log = f"Bước {nodes_expanded}: Xét node ({r},{c}) [g={cost}], còn {len(dirty)} ô bụi"
        ...
        for dr, dc in directions:
            ...
            if 0 <= nr < M and 0 <= nc < N and (nr, nc) not in obstacles:
                ...
                if next_state not in best_cost or next_cost < best_cost[next_state]:
                    ...
                    tie_breaker += 1
                    heapq.heappush(frontier, (next_cost, tie_breaker, next_state))
...
def solve_ucs_stepwise(M, N, start_pos, initial_dirty, obstacles, style):
    ...
    while frontier:
        ...
        frontier_show = ", ".join(f"({s[2][0][0]},{s[2][0][1]})[g={s[0]}]" for s in sorted_frontier[:5])
        if len(frontier) > 5:
            frontier_show += f" ... (+{len(frontier)-5} node)"

        log_text = f"Bước {nodes_expanded}: Xét node ({r},{c}) [g={cost}], còn {len(dirty)} ô bụi\n  -> Frontier: [{frontier_show}]"
